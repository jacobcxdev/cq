...
Final code-gate evidence for commit `38ec5b2` used Go `go1.26.5 darwin/arm64` and Desktop `0.147.0-alpha.6.5`: `go test -race -count=1 ./...`, `go vet ./...`, `go build ./...`, and `git diff --check` passed. Targeted turn lifecycle, continuation, resynchronisation, shared-transport lease, credential writer, and candidate-refresh suites also passed 100 repeated race-enabled runs; refresh overlap proof passed 1,000 repeated race-enabled runs. Installed acceptance marker remained temporary and did not activate live enforcement. Final documentation commits do not alter installed code evidence.
