...
- [ ] Add policy red tests. Admitted account A becomes the lane's successor task affinity; before the installed `gpt-5.6-sol` 30-minute floor A beats higher-headroom B, and at the boundary deterministic fairness becomes eligible. Unknown model policy, missing timing, clock rollback, and hard continuation stay sticky. Model-domain change or authoritative hard quota may release only soft task affinity; missing, incompatible, or unroutable warm authority fails closed. A provisional/failed-unadmitted choice establishes no affinity, including after restart.
...
- [ ] Prove Stage 12A task affinity prevents turn-by-turn load balancing before the versioned cache floor, slides on each successfully admitted cache-eligible request, restores after restart, fails closed on unknown timing/policy, permits deterministic max-min fairness only at/after the floor or exact hard quota, ignores system `Active`, and dispatches the terminal routing default at most once after ordinary alternatives.
- [ ] Prove exact HTTP hard-429 replay occurs only for a never-admitted soft/unbound lease before downstream bytes; a hard-bound lease or later same-turn hard 429 after any prior admission dispatches no alternate/default.
