...
When canonical start replaces a valid completed run, it holds both the canonical publisher lock and existing serving-owner lock. Before publishing the new run, it retires only exact signed stop artifacts matching that completed run's finalisation digest, after validating every present artifact and retaining its filesystem identity through removal and directory sync. Orphan, malformed, mismatched or replaced artifacts prevent a new run and are never removed as general cleanup. Retirement or subsequent publication failure leaves the old signed finalisation available; already retired intent is not recreated. A retry may proceed when the old record remains valid and its matching artifacts have already been retired. Lock-release failure is reported even if publication has occurred; inspect retained state before retrying.
