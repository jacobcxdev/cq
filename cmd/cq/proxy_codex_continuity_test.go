...
func TestNewProxyCodexNativeHTTPInstallsForEnforcementAndRetainedAuthority(t *testing.T) {
	...
	if planner == nil || planner.Inventory != dependency || planner.Capacity != capacity || planner.Routes != dependency || planner.Runtime != dependency || planner.DefaultAccountKey != "default-account" || planner.PinnedAccountKey != "pinned-account" || planner.Authority.ModeEpoch != 9 || !planner.Authority.Authoritative || !reflect.DeepEqual(planner.Authority.RetainedAuthoritativeEpochs, retained) {
...
func TestNewProxyCodexWebSocketInstallsOnlyForEnforcement(t *testing.T) {
	...
	handler, err := newProxyCodexWebSocket(proxyCodexWebSocketDependencies{
		...
		DefaultAccountKey: "default-account", PinnedAccountKey: "pinned-account", Executor: executor, Upstream: "https://codex.example/backend-api", Now: time.Now,
	...
	if planner == nil || planner.Inventory != dependency || planner.Capacity != capacity || planner.Routes != dependency || planner.Runtime != dependency || planner.DefaultAccountKey != "default-account" || planner.PinnedAccountKey != "pinned-account" || planner.Authority.ModeEpoch != 12 || !planner.Authority.Authoritative {
