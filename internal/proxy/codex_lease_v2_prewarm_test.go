...
func TestCodexPrewarmAdoptionMarkerCannotBeForgedOrChangedByGenericCAS(t *testing.T) {
	...
	if _, _, _, err := coordinator.store.buildCodexLeaseRecordAfterImage(CodexJournalRecordV2{}, false, forged, false, false, false, CodexLeaseRecordFence{}, coordinator.store.policy.Now()); err == nil {
	...
	if _, _, _, err := coordinator.store.buildCodexLeaseRecordAfterImage(result.Record, true, changed, false, false, false, fence, coordinator.store.policy.Now()); err == nil {
